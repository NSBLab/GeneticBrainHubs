  function rgb = vals2colormap(vals, cmap, crange)
...
if ~iscell(vals)
    ...
    valsN = round(((vals - crange(1)) ./ diff(crange)) .* M)+1;
...
elseif iscell(vals)
    ...
    for ii = 1:length(vals)
        ...
        valsN = round(((valsN - crange(1)) ./ diff(crange)) .* M)+1;
        % Convert any nans to ones
        valsN(isnan(valsN)) = 1;
        % Convert the normalized values to the RGB values of the colormap
        rgb{ii} = cmap(valsN, :);
