function [pORA_eQTL, genes_ENTREZ] = extract_null_eQTLs(whatDATA, whatSET)
...
% the number of SNPs selected at random will be the same as the number of target SNPs in the real dataset
% 2.1 select random SNPs from the list of clumped SNPs
numSNPs = size(targetSNP,1); 
randIND = randsample(size(SNPIDs,1),numSNPs); 

% 3. get rs IDs for those random SNPs
rand_targetSNPrs = SNPIDs.Var1(randIND); 
