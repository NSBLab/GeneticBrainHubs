...
if plotFig > 0
    ...
    %figure
    imagesc(plotMat, [0, max(plotMat(:))]); % Plot matrix, with limits between 0 and max val in nmatrix
    %colormap(hot);
    hold on
    set(gca,'XTick',1:1:length(labels)); % Set x tick vals
    set(gca,'XTickLabel',labels,'FontSize',16,'FontWeight','Bold','XTickLabelRotation',45); % set x tick labels
    set(gca,'YTick',1:1:length(labels)); % set y tick vals
    set(gca,'YTickLabel',labels,'FontSize',16,'FontWeight','Bold'); % set y tick labels
    %colormap jet
    colorbar; % show colour bar
    h=gcf; % get the figure handle
    set(gcf,'Color','w');

    % plot values in lower triangle of matrix
    for i=1:size(plotMat,1)
       for j=i:size(plotMat,2)
            text(i,j,num2str(plotMat(i,j),2),'HorizontalAlignment','center',...
